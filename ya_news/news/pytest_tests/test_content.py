...
def test_authorized_client_has_form(author_client, news_list):
    response = author_client.get(news_list)
    assert FORM_NAME in response.context
    assert isinstance(response.context[FORM_NAME], CommentForm)


def test_anonymous_client_has_no_form(client, news_list):
    response = client.get(news_list)
    assert FORM_NAME not in response.context


def test_comments_order(client, news_list, comments):
    response = client.get(news_list)
...
def test_news_count(client, news, news_list):
